...
export default function SignMeritoriaWithToken() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const res = await fetch(`${getApiBase()}/sign/meritoria/token/${token}`);
  ...
  const handleUploadPdf = async () => {
    ...
    try {
      const res = await fetch(`${getApiBase()}/sign/meritoria/token/${token}/upload-signed`, {
        method: 'POST',
        body: formData,
      });
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 sm:p-6 bg-card rounded-lg shadow">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground mb-4">Por favor descargue, firme y suba el PDF.</p>
      </div>
